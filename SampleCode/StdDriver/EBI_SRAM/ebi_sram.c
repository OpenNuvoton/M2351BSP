...
int32_t SRAM_BS616LV4017(uint32_t u32MaxSize);
...
int32_t SRAM_CompareFunction(uint32_t u32Param0, uint32_t u32Param1, uint32_t u32Param2, uint32_t u32Param3)
{
    ...
    if(u32Param0 == 4)      // 0x0, 0x1, 0x2, 0x3, 0x4 ... access with offset ......
    {
        ...
        while(u32Idx < u32EBIsize)
        {
            ...
            {
                u8D0 = EBI0_READ_DATA8(EBI, u32Idx);
                u8D1 = EBI0_READ_DATA8(EBI, u32Idx + 1);
            ...
            else if(u32WriteMode == 2)      // From One Word Write Mode
            {
                u8D0 = EBI0_READ_DATA8(EBI, u32Idx);
                u8D1 = EBI0_READ_DATA8(EBI, u32Idx + 1);
                u8D2 = EBI0_READ_DATA8(EBI, u32Idx + 2);
                u8D3 = EBI0_READ_DATA8(EBI, u32Idx + 3);
        ...
        while(u32Idx < u32EBIsize)
        {
            if(u32WriteMode == 0)           // From Byte Write Mode
            {
                ...
                u32CmpData = (uint16_t)(u8D0 + (u8D1 << 8));
                if(EBI0_READ_DATA16(EBI, u32Idx) != (uint16_t)(u32CmpData))
                {
                    printf("\nSRAM compare fail, Addr= 0x%x\n", (uint32_t)EBI_BANK0_BASE_ADDR + u32Idx);
                    printf("SRAM want data= 0x%X\n", (uint16_t)(u32CmpData));
                    printf("SRAM read data= 0x%X\n", EBI0_READ_DATA16(EBI, u32Idx));
            ...
            {
                u16D0 = EBI0_READ_DATA16(EBI, u32Idx);
                u16D1 = EBI0_READ_DATA16(EBI, u32Idx + 2);
        ...
        while(u32Idx < u32EBIsize)
        {
            if(u32WriteMode == 0)           // From Byte Write Mode
            {
                ...
                u32CmpData = (uint32_t)(u8D0 + (u8D1 << 8) + (u8D2 << 16) + (u8D3 << 24));
                if(EBI0_READ_DATA32(EBI, u32Idx) != (uint32_t)(u32CmpData))
                {
                    printf("\nSRAM compare fail, Addr= 0x%x\n", (uint32_t)EBI_BANK0_BASE_ADDR + u32Idx);
                    printf("SRAM want data= 0x%X\n", (uint32_t)(u32CmpData));
                    printf("SRAM read data= 0x%X\n", EBI0_READ_DATA32(EBI, u32Idx));
...
int32_t SRAM_BS616LV4017(uint32_t u32MaxSize)
{
    ...
    while(u32DataIdx < 5)
    {
        if(u32DataIdx < 4)
        {
            ...
            while(u32Idx < u32EBIsize)
            {
                EBI0_WRITE_DATA8(EBI, u32Idx, (uint8_t)(u32WriteData));
                u32Idx++;
            }
        ...
        else
        {
            ...
            while(u32Idx < u32EBIsize)
            {
                EBI0_WRITE_DATA8(EBI, u32Idx, (uint8_t)(u32WriteData + (uint32_t)&u32Idx));
                u32Idx++;
                u32WriteData++;
            }
    ...
    while(u32DataIdx < 5)
    {
        if(u32DataIdx < 4)
        {
            ...
            while(u32Idx < u32EBIsize)
            {
                EBI0_WRITE_DATA16(EBI, u32Idx, (uint16_t)(u32WriteData));
                u32Idx += 2;
            }
        ...
        else
        {
            ...
            while(u32Idx < u32EBIsize)
            {
                EBI0_WRITE_DATA16(EBI, u32Idx, (uint16_t)(u32WriteData + (uint32_t)&u32Idx));
                u32Idx += 2;
                u32WriteData++;
            }
    ...
    while(u32DataIdx < 5)
    {
        if(u32DataIdx < 4)
        {
            ...
            while(u32Idx < u32EBIsize)
            {
                EBI0_WRITE_DATA32(EBI, u32Idx, (uint32_t)(u32WriteData));
                u32Idx += 4;
            }
        ...
        else
        {
            ...
            while(u32Idx < u32EBIsize)
            {
                EBI0_WRITE_DATA32(EBI, u32Idx, (uint32_t)(u32WriteData + (uint32_t)&u32Idx));
                u32Idx += 4;
                u32WriteData++;
            }
