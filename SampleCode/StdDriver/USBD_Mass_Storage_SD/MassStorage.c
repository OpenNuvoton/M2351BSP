...
void MSC_Init(void)
{
    ...
    {
        g_u32TotalSectors = 0;
        s_au8SenseKey[0] = 0x03;
        s_au8SenseKey[1] = 0x30;
        s_au8SenseKey[2] = 0x01;
    }
...
void MSC_ReadFormatCapacity(void)
{
    ...
    pu8Desc[4] = *((uint8_t *)&g_u32TotalSectors + 3);
    pu8Desc[5] = *((uint8_t *)&g_u32TotalSectors + 2);
    pu8Desc[6] = *((uint8_t *)&g_u32TotalSectors + 1);
    pu8Desc[7] = *((uint8_t *)&g_u32TotalSectors + 0);
    ...
    // Number of Blocks
    pu8Desc[12] = *((uint8_t *)&g_u32TotalSectors + 3);
    pu8Desc[13] = *((uint8_t *)&g_u32TotalSectors + 2);
    pu8Desc[14] = *((uint8_t *)&g_u32TotalSectors + 1);
    pu8Desc[15] = *((uint8_t *)&g_u32TotalSectors + 0);
    ...
    switch(s_sCBW.au8Data[0])
    {
        ...
        case 0x05:
            *((uint8_t *)MassCMD_BUF) = 39;
            ...
            NumHead = 2;
            NumSector = 64;
            NumCyl = (uint16_t)(g_u32TotalSectors / 128);
...
void MSC_ProcessCmd(void)
{
    ...
    if(s_u8EP3Ready)
    {
        ...
        if(s_u8BulkState == BULK_CBW)
        {
            ...
            Hcount = s_sCBW.dCBWDataTransferLength;
            ...
            switch(s_sCBW.u8OPCode)
            {
                ...
                case UFI_MODE_SENSE_6:
                {
                    ...
                    *(uint8_t *)((uint32_t)USBD_BUF_BASE + s_u32BulkBuf1 + 3) = 0x0;
                    ...
                    s_u8BulkState = BULK_IN;
                    s_sCSW.bCSWStatus = 0;
                    s_sCSW.dCSWDataResidue = Hcount - 4;
                    return;
                }
                ...
                case UFI_INQUIRY:
                {
                    ...
                    {
                        /* Bulk IN buffer */
                        USBD_MemCopy((uint8_t *)((uint32_t)USBD_BUF_BASE + s_u32BulkBuf1), (uint8_t *)s_au8InquiryID, Hcount);
                        USBD_SET_PAYLOAD_LEN(EP2, Hcount);
                        s_u8BulkState = BULK_IN;
                        s_sCSW.bCSWStatus = 0;
                        s_sCSW.dCSWDataResidue = 0;
                ...
                case UFI_READ_10:
                {
                    ...
                    if(s_u32Length == 0)
                    {
                        ...
                        Dcount = (get_be32(&s_sCBW.au8Data[4]) >> 8) * 512;
...
void MSC_AckCmd(void)
{
    ...
    else if(s_u8BulkState == BULK_IN)
    {
        ...
        {
            if((s_sCBW.u8OPCode == UFI_INQUIRY) || (s_sCBW.u8OPCode == UFI_REQUEST_SENSE))
                s_sCSW.bCSWStatus = 0x00;
            else
                s_sCSW.bCSWStatus = 0x01;
        }
        /* Return the CSW */
