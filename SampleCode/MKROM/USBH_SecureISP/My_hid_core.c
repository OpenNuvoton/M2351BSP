                             ...
                             uint8_t *data, int len)
{
    ...
    ret = usbh_ctrl_xfer(iface->udev,
                         ...
                         (uint16_t)(rtp_id + (rtp_typ << 8)),   /* wValue                                */
                         iface->if_num,                         /* wIndex                                */
        ...
        uint8_t *data, int len)
{
    ...
    utr->setup.wValue     = (uint16_t)(rtp_id + (rtp_typ << 8));
    utr->setup.wIndex     = iface->if_num;
    utr->setup.wLength    = (uint16_t)len;

    utr->buff = data;
    utr->data_len = (uint32_t)len;
...
int32_t  usbh_hid_set_idle(HID_DEV_T *hdev, int rtp_id, uint8_t idle_rate)
{
    ...
    ret = usbh_ctrl_xfer(iface->udev,
                         ...
                         (uint16_t)(rtp_id + (wValue << 8)),    /* wValue                                */
                         iface->if_num,                         /* wIndex                                */
                         0,                                     /* wLength                               */
...
static void  hid_read_irq(UTR_T *utr)
{
    ...
    if(ret)
    {
        ...
        usbh_free_mem(utr->buff, (int32_t)utr->data_len);
        free_utr(utr);
    }
}
...
int32_t usbh_hid_start_int_read(HID_DEV_T *hdev, uint8_t ep_addr, HID_IR_FUNC *func)
{
    ...
    {
        utr = hdev->utr_list[i];
        if((utr != NULL) && (utr->ep != NULL) && ((utr->ep->bEndpointAddress == ep_addr) || (ep_addr == 0)))
        {
