...
)
{
    ...
    if(pdrv == 0)
    {
        if(shift_buf_flag == 1)
        {
            if(count == 1)
            {
                u32Ret = SDH_Read(pSDH, (uint8_t*)(&s_au32TmpBuffer), sector, count);
                memcpy(buff, (&s_au32TmpBuffer), count * (UINT)SD0.sectorSize);
            }
            else
            {
                tmp_StartBufAddr = (((uint32_t)buff / 4 + 1) * 4);
                u32Ret = SDH_Read(pSDH, ((uint8_t*)tmp_StartBufAddr), sector, (count - 1));
                memcpy(buff, (void*)tmp_StartBufAddr, ((UINT)SD0.sectorSize * (count - 1)));
                u32Ret = SDH_Read(pSDH, (uint8_t*)(&s_au32TmpBuffer), (sector + count - 1), 1);
                memcpy((buff + ((UINT)SD0.sectorSize * (count - 1))), (void*)s_au32TmpBuffer, (uint32_t)SD0.sectorSize);
            }
        }
        else
            u32Ret = SDH_Read(SDH0, buff, sector, count);
    }
//    else if (pdrv == 1)
//        ret = (DRESULT) SDH_Read(SDH1, buff, sector, count);
    else
        u32Ret = RES_PARERR;
    return (DRESULT)u32Ret;
}
    ...
    if(pdrv == 0)
    {
        if(shift_buf_flag == 1)
        {
            if(count == 1)
            {
                ...
                u32Ret = SDH_Write(pSDH, (uint8_t*)(&s_au32TmpBuffer), sector, count);
            }
            else
            {
                tmp_StartBufAddr = (((uint32_t)buff / 4 + 1) * 4);
                ...
                u32Ret = SDH_Write(pSDH, ((uint8_t*)tmp_StartBufAddr), sector, (count - 1));
                u32Ret = SDH_Write(pSDH, (uint8_t*)(&s_au32TmpBuffer), (sector + count - 1), 1);
            }
        }
        else
            u32Ret = SDH_Write(pSDH, (uint8_t *)(uint32_t)buff, sector, count);
    }
//    else if (pdrv == 1)
//        u32Ret = SDH_Write(SDH1, (uint8_t *)buff, sector, count);
    else
        u32Ret = RES_PARERR;
